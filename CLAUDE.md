...
- Use `$derived.by(() => {...})` for any multi-line derived computation. `$derived(expr)` is for single-expression values only — passing a function to `$derived()` stores the function, not its result.
- Component internal IDs (e.g. `uid` from `useId()`) must be used consistently for both the `id` attribute on the input and the `for` attribute on the label. Never use a prop that may be `undefined` for those attributes.
...
- Every user-visible string must go through Paraglide — including `aria-label`, `title`, and tooltip attributes. No hardcoded strings anywhere in the component tree.
...
- Never cast `char as u8` without a prior `c.is_ascii()` guard (or use `u8::try_from(c)`). Truncation is silent and corrupts VT parsing.
- Never downcast a trait object via raw pointer (`*mut dyn Trait as *mut ConcreteType`). Add the required method to the trait instead, or use `Any::downcast_mut`.
- Never mutate a `DashMap` entry with `remove` + `insert`. Use `get_mut()` to avoid the race window between the two operations.
- Never use `try_lock().expect(...)` on async mutexes — use `.lock().await`.
- Guard against `usize` underflow before subtraction: check that the value is non-zero (or the collection is non-empty) before subtracting.
- Never log filesystem paths that include usernames or home directories. Log the filename only (e.g. `"preferences.json"`, not `path.display()`).

### IPC event rules

- Backend events that affect a specific entity (tab, pane, session) must include the entity's ID explicitly in the payload (e.g. `closed_tab_id`, not just `active_tab_id`). Frontend handlers must use that ID to locate and update the correct entity.
