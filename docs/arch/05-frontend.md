...
src/
  ...
  lib/
    ...
    generated/                         — build-time artifacts (gitignored)
      third-party-licenses.json       — merged Rust + npm dependency licenses;
                                        generated by scripts/generate-licenses.sh
                                        via vite-plugin-licenses.ts at each build
  ...
  components/
    ...
    StatusBar.svelte                 — CWD, SSH indicator, version label, settings button
    AboutPopover.svelte              — version label trigger + non-modal popover (license, third-party notices)
