...
- **Content:** Left-aligned: active pane current working directory (truncated with ellipsis if too long). Right-aligned (in order): terminal dimensions (cols × rows) — transient: visible only during pane resize, fades out after a 2s delay (see §9.3); SSH connection status (if applicable); version label — About popover trigger (see §7.24); Settings button.
- **Version label:** The application version string (e.g., "0.1.0") rendered in `--font-mono-ui`, `--font-size-ui-xs` (11px), `--color-text-muted`. Clicking opens the About popover (§7.24). The visible text label acts as the trigger; the actual hit area is expanded to 44×44px via a CSS pseudo-element so as not to distort the status bar height. `tabindex="-1"` — not in the primary tab order (the status bar is not a keyboard-navigation surface; the About popover is accessible via the Preferences panel §7.23).
