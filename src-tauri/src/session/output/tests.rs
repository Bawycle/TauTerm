...
//! Unit tests for the shared emit pipeline (`session/output`).
//!
//! Migrated verbatim from `session/pty_task/tests.rs` (Commit 1, ADR-0028) —
//! function names preserved so test IDs remain discoverable in nextest reports.
//! Imports rewritten from `super::reader::*` to `super::coalescer::*`.
//!
//! Test ID groups covered here:
//!
//! - TEST-ACK-001..021 — frame-ack two-stage backpressure (ADR-0027)
//! - TEST-ADPT-001..005 — adaptive debounce clamping (P-HT-2)
//! - TEST-PIPC2-UNIT-001..003 — `ProcessOutput::merge` invariants
//! - TEST-SB-VIEWPORT-001..005 — composite scrollback viewport builder
//! - `screen_update_event_serde_roundtrip*` — IPC contract pin
//! - COAL-MERGE-001..005 — NEW: per-field merge invariants for the 6
//!   non-`dirty` / non-`needs_immediate_flush` `ProcessOutput` fields
//!   (gap identified by test-engineer pre-ADR-0028).
...
use super::coalescer::{
    ACK_DROP_THRESHOLD_MS, ACK_STALE_DEBOUNCE, ACK_STALE_THRESHOLD_MS, CoalescerConfig,
    DEBOUNCE_MAX, DEBOUNCE_MIN, next_debounce, now_ms,
};
...
fn screen_update_event_serde_roundtrip() {
    ...
    let decoded: ScreenUpdateEvent = serde_json::from_str(&json).expect("deserialization failed");
...
fn next_debounce_zero_clamps_to_min() {
    assert_eq!(
        next_debounce(Duration::ZERO, &CoalescerConfig::pty_default()),
        DEBOUNCE_MIN
    );
}
...
fn next_debounce_small_clamps_to_min() {
    assert_eq!(
        next_debounce(Duration::from_millis(5), &CoalescerConfig::pty_default()),
        DEBOUNCE_MIN
    );
}
...
fn next_debounce_large_clamps_to_max() {
    assert_eq!(
        next_debounce(Duration::from_millis(200), &CoalescerConfig::pty_default()),
        DEBOUNCE_MAX
    );
}
...
fn next_debounce_scales_correctly() {
    assert_eq!(
        next_debounce(Duration::from_millis(20), &CoalescerConfig::pty_default()),
...
fn next_debounce_near_max_boundary() {
    let d83 = next_debounce(Duration::from_millis(83), &CoalescerConfig::pty_default());
    assert!(d83 < DEBOUNCE_MAX);
    assert!(d83 > DEBOUNCE_MIN);

    let d84 = next_debounce(Duration::from_millis(84), &CoalescerConfig::pty_default());
...
fn exit_drop_mode_forces_full_redraw() {
    ...
    assert!(
        ...
        "exiting drop mode must force full redraw"
    );
}
...
fn drop_exit_via_ack_arrival_forces_full_redraw() {
    ...
    // Simulate the transition logic from coalescer.rs.
    let mut pending = ProcessOutput {
        dirty: DirtyRegion {
            rows: Default::default(),
            is_full_redraw: false,
...
/// The exact conditional pattern used at the 3 call sites in `coalescer.rs`
...
/// at the moment of the transition), the coalescer loop MUST NOT invoke
/// `emit_all_pending` and MUST NOT synthesize a full redraw for an empty
/// `pending`. This mirrors the `!pending.is_empty()` control-flow guard in
/// the timer arm of `coalescer::run`.
#[test]
fn was_in_drop_mode_transition_with_empty_pending_skips_emit() {
    ...
    // Structural guard from coalescer.rs (outer `if !pending.is_empty()`).
...
fn bell_flood_does_not_escalate_to_stale_debounce() {
    ...
    for i in 0..10u64 {
        ...
        // Gated assignment (matches coalescer.rs).
        if emitted_screen_update {
        ...
        let current_debounce = if in_stale_mode {
            ...
            next_debounce(Duration::from_micros(50), &CoalescerConfig::pty_default())
        };
        assert!(
...
// -----------------------------------------------------------------------
// TEST-ACK-021 — ack during idle preserves was_in_drop_mode for next
// active tick, ensuring full redraw is applied.
// -----------------------------------------------------------------------

/// Scenario: pane is in drop mode (stage 2), frontend sends ack while no
/// data is pending (idle tick). On the next tick with actual dirty data,
/// the coalescer must detect the drop→normal transition and force
/// `is_full_redraw = true`. This validates that `was_in_drop_mode` is NOT
/// cleared on idle ticks.
#[test]
fn ack_during_idle_preserves_drop_transition_for_next_active_tick() {
    let base_time: u64 = 1_700_000_000_000;

    // --- Tick N: pane is in drop mode, pending is empty (idle tick) ---

    let last_emit_ms: u64 = base_time; // emitted screen update earlier
    let last_ack_ms_before_ack: u64 = base_time - 2_000; // ack is 2 s stale
    let now_tick_n: u64 = base_time + 100;

    let ack_age_ms = now_tick_n.saturating_sub(last_ack_ms_before_ack);
    let has_unacked_emits = last_emit_ms > last_ack_ms_before_ack;
    let in_drop_mode_tick_n = has_unacked_emits && ack_age_ms > ACK_DROP_THRESHOLD_MS;

    assert!(
        in_drop_mode_tick_n,
        "precondition: pane must be in drop mode at tick N"
    );

    // was_in_drop_mode tracks the previous active tick's state.
    // At this point it is true (set during the last active tick that was
    // also in drop mode).
    let mut was_in_drop_mode = true;

    // Tick N is idle (pending is empty). Per the fix, was_in_drop_mode
    // must NOT be updated.
    let pending_tick_n = ProcessOutput::default();
    assert!(pending_tick_n.is_empty(), "precondition: tick N is idle");

    // Simulate the fixed coalescer logic for idle tick:
    // The active branch is NOT entered → was_in_drop_mode stays unchanged.
    // (Before the fix, `was_in_drop_mode = in_drop_mode` ran unconditionally
    // and would remain `true` here — but if in_drop_mode had become `false`
    // due to the ack arriving mid-tick, it would have been cleared.)

    // --- Between ticks: frontend ack arrives ---
    let last_ack_ms_after_ack: u64 = now_tick_n + 50; // ack arrives

    // --- Tick N+1: data arrives, ack is fresh ---
    let now_tick_n1: u64 = now_tick_n + 80;

    let ack_age_ms_n1 = now_tick_n1.saturating_sub(last_ack_ms_after_ack);
    let has_unacked_emits_n1 = last_emit_ms > last_ack_ms_after_ack;
    let in_drop_mode_tick_n1 = has_unacked_emits_n1 && ack_age_ms_n1 > ACK_DROP_THRESHOLD_MS;

    assert!(
        !in_drop_mode_tick_n1,
        "postcondition: ack arrived, pane must NOT be in drop mode at tick N+1"
    );

    // Dirty data arrives at tick N+1.
    let mut pending_tick_n1 = ProcessOutput {
        dirty: DirtyRegion {
            is_full_redraw: false,
            cursor_moved: true, // cursor moved — makes pending non-empty
            ..Default::default()
        },
        ..Default::default()
    };
    ...
    assert!(
        !pending_tick_n1.is_empty(),
        "precondition: tick N+1 has dirty data"
    );

    // Apply coalescer transition logic (mirrors coalescer.rs active path).
    if in_drop_mode_tick_n1 {
        pending_tick_n1.dirty = DirtyRegion::default();
    } else if was_in_drop_mode {
        pending_tick_n1.dirty.is_full_redraw = true;
    }

    assert!(
        pending_tick_n1.dirty.is_full_redraw,
        "exiting drop mode after idle ack must force full redraw on next active tick"
    );

    // After the active tick, was_in_drop_mode is updated.
    was_in_drop_mode = in_drop_mode_tick_n1;
    assert!(
        !was_in_drop_mode,
        "after active tick N+1, was_in_drop_mode must reflect current state"
    );
}

// =======================================================================
// COAL-MERGE-001..005 — NEW per-field ProcessOutput::merge invariants
//
// Pre-existing TEST-PIPC2-UNIT-001..003 only cover `needs_immediate_flush`
// and (indirectly) `dirty`. ADR-0028 §5 surfaces 5 more fields whose merge
// semantics MUST be exercised so future SSH-side reuse cannot regress them
// silently. Gap identified by test-engineer (Commit 1 plan, Decisions §1).
// =======================================================================

// -----------------------------------------------------------------------
// COAL-MERGE-001 — bell merges via OR
// -----------------------------------------------------------------------

/// `bell` must OR across merges: any bell in the window is preserved.
/// Verifies all 4 truth-table combinations.
#[test]
fn coal_merge_001_bell_merges_via_or() {
    // false | false = false
    let mut a = ProcessOutput::default();
    a.merge(ProcessOutput::default());
    assert!(!a.bell, "false | false must remain false");

    // false | true = true
    let mut a = ProcessOutput::default();
    a.merge(ProcessOutput {
        bell: true,
        ..Default::default()
    });
    assert!(a.bell, "false | true must become true");

    // true | false = true (sticky)
    let mut a = ProcessOutput {
        bell: true,
        ..Default::default()
    };
    a.merge(ProcessOutput::default());
    assert!(a.bell, "true | false must remain true (sticky)");
    ...
    assert!(a.bell, "true | true must remain true");
}

// -----------------------------------------------------------------------
// COAL-MERGE-002 — mode_changed merges via OR
// -----------------------------------------------------------------------

/// `mode_changed` must OR across merges: any mode change in the window is
/// preserved (ensures mode-state-changed event is emitted exactly once).
#[test]
fn coal_merge_002_mode_changed_merges_via_or() {
    // false | true = true
    let mut a = ProcessOutput::default();
    a.merge(ProcessOutput {
        mode_changed: true,
    ...
    assert!(a.mode_changed, "false | true must become true");
    ...
    assert!(
        a.mode_changed,
        "true | false must remain true (sticky across multiple chunks)"
    );
}

// -----------------------------------------------------------------------
// COAL-MERGE-003 — osc52 last-wins
// -----------------------------------------------------------------------

/// `osc52` must follow last-wins semantics: the most recent OSC 52 payload
/// is preserved (matches semantics of "set clipboard"), and `None` does
/// NOT clear an existing payload (sticky-Some).
#[test]
fn coal_merge_003_osc52_last_wins() {
    // Some(A) merged with Some(B) → Some(B)
    let mut a = ProcessOutput {
        osc52: Some("first".to_string()),
        ..Default::default()
    };
    a.merge(ProcessOutput {
        osc52: Some("second".to_string()),
        ..Default::default()
    });
    assert_eq!(
        a.osc52.as_deref(),
        Some("second"),
        "Some(A) | Some(B) must yield Some(B) — last-wins"
    );

    // Some(A) merged with None → Some(A) (None does not clear)
    let mut a = ProcessOutput {
        osc52: Some("kept".to_string()),
        ..Default::default()
    };
    a.merge(ProcessOutput::default());
    assert_eq!(
        a.osc52.as_deref(),
        Some("kept"),
        "Some(A) | None must yield Some(A) — None must not overwrite"
    );

    // None merged with Some(B) → Some(B)
    let mut a = ProcessOutput::default();
    a.merge(ProcessOutput {
        osc52: Some("only".to_string()),
        ..Default::default()
    });
    assert_eq!(
        a.osc52.as_deref(),
        Some("only"),
        "None | Some(B) must yield Some(B)"
    );
}

// -----------------------------------------------------------------------
// COAL-MERGE-004 — new_cwd last-wins
// -----------------------------------------------------------------------

/// `new_cwd` must follow last-wins semantics (same contract as `osc52` and
/// `new_title`): the most recent OSC 7 cwd is preserved, and `None` does
/// NOT clear an existing cwd.
#[test]
fn coal_merge_004_new_cwd_last_wins() {
    // Some(A) merged with Some(B) → Some(B)
    let mut a = ProcessOutput {
        new_cwd: Some("/home".to_string()),
        ..Default::default()
    };
    a.merge(ProcessOutput {
        new_cwd: Some("/tmp".to_string()),
    ...
    assert_eq!(
        a.new_cwd.as_deref(),
        Some("/tmp"),
        "cwd: Some(A) | Some(B) must yield Some(B)"
    );

    // Some(A) merged with None → Some(A)
    let mut a = ProcessOutput {
        new_cwd: Some("/var".to_string()),
        ..Default::default()
    };
    a.merge(ProcessOutput::default());
    assert_eq!(
        a.new_cwd.as_deref(),
        Some("/var"),
        "cwd: Some(A) | None must yield Some(A) — None must not overwrite"
    );
}

// -----------------------------------------------------------------------
// COAL-MERGE-005 — new_cursor_shape last-wins
// -----------------------------------------------------------------------

/// `new_cursor_shape` must follow last-wins semantics: the most recent
/// DECSCUSR shape is preserved, and `None` does NOT clear an existing shape.
#[test]
fn coal_merge_005_new_cursor_shape_last_wins() {
    // Some(A) merged with Some(B) → Some(B)
    let mut a = ProcessOutput {
        new_cursor_shape: Some(1),
        ..Default::default()
    };
    a.merge(ProcessOutput {
        new_cursor_shape: Some(4),
    ...
    assert_eq!(
        a.new_cursor_shape,
        Some(4),
        "cursor_shape: Some(1) | Some(4) must yield Some(4)"
    );
    ...
    assert_eq!(
        a.new_cursor_shape,
        Some(2),
        "cursor_shape: Some(2) | None must yield Some(2) — None must not overwrite"
    );
}
